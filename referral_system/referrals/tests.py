...
class ReferralTests(TestCase):
    ...
    def test_get_referral_code_valid_user(self):
        response = self.client.get(reverse('retrieve_referral', args=[self.referrer.email]))
    ...
    def test_get_referral_code_no_code(self):
        ...
        response = self.client.get(reverse('retrieve_referral', args=[self.referrer.email]))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(response.data['detail'], 'No active referral code for this user.')

    def test_get_referral_code_invalid_user(self):
        response = self.client.get(reverse('retrieve_referral', args=['invalid@example.com']))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(response.data['detail'], 'User not found.')

    def test_register_with_valid_referral_code(self):
        response = self.client.post(reverse('create_referral'), {'referral_code': 'REFERRALCODE'})
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_register_with_invalid_referral_code(self):
        response = self.client.post(reverse('create_referral'), {'referral_code': 'INVALIDCODE'})
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
